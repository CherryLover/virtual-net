...
export async function load(): Promise<SavedTopology | null> {
  ...
  const parsed = parseTopology(saved?.topology, { allowInvalidConfig: true });
...
export async function backupBeforeImport(topology: Topology): Promise<void> {
  const database = await db();
  await database.put(STORE, { topology, savedAt: Date.now() }, "before-import");
}

export async function loadImportBackup(): Promise<SavedTopology | null> {
  const database = await db();
  const saved = await database.get(STORE, "before-import");
  if (!saved) return null;
  const parsed = parseTopology(saved.topology, { allowInvalidConfig: true });
  if (!parsed.ok) throw new Error("导入前备份无法读取，原始数据已保留。");
  return { ...saved, topology: parsed.topology };
}
