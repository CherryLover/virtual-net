...
export interface DnsProxySelection {
  deviceId: string;
  username?: string;
  password?: string;
}

/** DNS payloads travel through one per-probe SOCKS5 UDP association, not the TCP control flow. */
export function socksDns(
  topology: Topology,
  runtime: Runtime,
  options: { sourceDeviceId: string; domain: string; server: string; proxy: DnsProxySelection },
): ProbeResult {
  ...
  if (parseIp(options.server) === null) {
  ...
  if (
    !segment("proxy-auth", options.sourceDeviceId, proxy.id, () => {
      if (cfg.protocol !== "socks5")
        return fail(proxy.id, "PROXY_PROTOCOL_MISMATCH", "UDP 查询需要 SOCKS5 代理");
      if (
        cfg.auth === "password" &&
  ...
  if (
    !segment("proxy-associate", options.sourceDeviceId, ip, () => {
      ...
      walk.log.pass({
        ...
        note: `UDP ASSOCIATE：中继地址 ${ip}:${relayPort}，关联仅用于本次查询`,
      });
  ...
  if (
    !segment(
      ...
      proxy.id,
      options.sourceDeviceId,
      () => walk.returnResponse(proxy.id, "udp").ok,
    )
  )
    return finish();
  dns = answer;
