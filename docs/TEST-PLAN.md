...
| R-20260907-14 | 网页 / 组件 / 页面 | 一次拖动一次历史，刷新和文件往返保留，新程序兼容旧图 | 第 6–7 节 | 拖动/撤销/重做/刷新/实际导出通过；导入组件事件与旧图测试通过；系统选择框全流程待补 |
...
- 最终命令：`pnpm lint` 通过（219 文件），`pnpm test` 通过（引擎 145 项、网页 99 项，共 244 项），`pnpm build` 通过且包含两包 `typecheck`。构建仍提示单个 JS 包超过 500 kB（582.38 kB，gzip 181.92 kB），不影响运行；未为消除提示扩大本轮拆包范围。
...
- 文件选择框完整实测的阻碍已定位：Chrome 的 ChatGPT 扩展未开启“允许访问文件网址”，自动化设置所选文件返回 `Not allowed`；独立内置浏览器在当前环境不可用。此权限需用户明确开启，不由项目修改浏览器权限。允许后重跑实际文件选择、确认替换、结果核对与撤销；R14 在补完之前不记作全页面验收完成。组件事件测试不能冒充这个端到端流程。
