...
本轮最终结果：420 项测试（224 引擎、196 网页）、代码检查 277 文件、包含类型检查的构建通过。UI 一致性、选择分组、原有 UX 主流程三个浏览器脚本通过，无页面异常。

选择入口修订：419 项测试（224 引擎、195 网页）通过，新增右键菜单与默认拖框多选，详见 [分组直接操作](checkpoints/UX-groups.md)。实测脚本 `scripts/selection-smoke.mjs` 覆盖组合键增减选择、右键成组/拆组、撤销、框选、平移、菜单边界及手机工具栏；以下配色 416 项为前一轮记录。

最新配色补验：Radix Colors 六套马卡龙/柔雾方案；416 项测试（224 引擎、192 网页）、lint 269 文件与构建通过。`scripts/ux-smoke.mjs` 和 `scripts/palette-smoke.mjs` 实际通过桌面/手机、六套切换、旧色兼容及真实分享下载；图标与按钮常态/悬停文字对比度至少 4.5:1。证据 `/tmp/virtual-net-palettes/`，以下 415 项是前次修订记录。

依据：[分组](checkpoints/UX-groups.md)、[配色](checkpoints/UX-appearance.md)、[导出分享](checkpoints/UX-export.md)、[导入检查](checkpoints/UX-import.md)。本节覆盖本地新增交互，不代表学习模块、后续网络协议或线上发布完成。

配色范围已收窄为整体强调色，画布与节点固定白灰。T-UX2-004 至 006 及相关文件往返已按新范围重新验证通过；旧深色或填充色方案的数量单独保留为历史记录，不代替本轮证据。

| 编号 | 检查与预期 | 当前证据 |
| --- | --- | --- |
| T-UX2-001 | 单/多设备复制只带内部连线，身份独立、代理引用正确、手动地址保留，一次撤销 | 引擎与状态测试；Chrome 实际复制三成员组、检查独立 MAC 后撤销通过 |
| T-UX2-002 | 成组、改名、成员增删、整组移动、解组、保存和文件往返；不改变网络含义 | Chrome 实际成组、移出互联网、拖组、一次撤销/重做及刷新；分组与配色随真实 JSON 文件往返通过 |
| T-UX2-003 | 手机通过勾选完成多设备选择和建组，不依赖 Shift；取消不改选择 | SelectionDialog 6 项测试；390×844 勾选两台电脑并建立命名分组通过 |
| T-UX2-004 | 整体强调色预设、自选及恢复默认；应用一次撤销，取消/关闭不变；保存、刷新、导入导出及重做保留强调色 | 新版颜色与弹窗测试通过；主流程实际自选、取消、刷新及真实 JSON 往返通过 |
| T-UX2-005 | 工作台各区域使用统一强调色，画布和节点固定白灰；无深色及填充覆盖入口；旧字段可解析但不渲染，语义状态不只靠颜色 | 实测海蓝五处图标一致、固定白灰及旧填充不渲染；强调色与旧字段兼容专项通过 |
| T-UX2-006 | 弹窗阻断背景编辑快捷键，桌面/小屏不溢出，强调色控件与确认按钮可达 | 键盘保护测试通过；新版主流程在 1440×900、390×844 实际通过，无页面异常 |
| T-UX2-007 | 导出完整图的 SVG/PNG/PDF，中文、长组名、标签、曲线、配色完整，不带编辑状态或配置密码 | 导出专项测试与真实三格式文件、图片像素断言；最终记录见 UX-export |
| T-UX2-008 | 分享先预览再保存，品牌与可扫描二维码清楚，超宽/超高图不裁切；普通导出不带品牌 | 实际 PNG 通过 macOS Vision 识别产品网址；本轮切换采集 244 帧无跳动无滚动，实际输出 720×771 与 1440×1542，相关专项 16 项通过；详细证据见 UX-export |
| T-UX2-009 | 空图和超大图明确拒绝，取消、生成失败不改图；透明背景与导出预览细节可检查 | 导出尺寸与边界测试、真实浏览器快照不变检查；最终记录见 UX-export |
| T-UX2-010 | 导入依次检查，结构损坏/未知类型/缺失关联/超限阻止，错误配置提醒后保留原值 | 导入检查与解析测试；Chrome 坏 JSON 阻止、真实文件导入通过；200设备上限测试不截断 |
| T-UX2-011 | 备份失败/取消/重选/卸载不替换原图，重复确认防重入，原图改变重新确认 | check/commit/ImportDialog 与数据库专项测试通过，备份可独立下载 |
| T-UX2-012 | 旧文件兼容、导入成功一次撤销，后续保存状态真实、原备份不被覆盖 | 数据库与组件测试，Chrome 导出/导入/冷刷新验证分组配色保留通过 |

真实主流程可重跑 `scripts/ux-smoke.mjs`，通过 `PLAYWRIGHT_MODULE` 指定已安装的 Playwright，通过 `QA_URL` 指定独立本地页面。输出 `/tmp/virtual-net-ux-qa`。

2026-09-08 新版最终自动检查：引擎 27 文件 / 224 项、网页 37 文件 / 191 项，合计 **415 项全部通过**；`pnpm lint` 检查 268 文件通过，构建及类型检查通过，仅保留原有单包超过 500 kB 提示。新版 `scripts/ux-smoke.mjs` 在桌面 1440×900 与手机 390×844 实际全流程通过，覆盖海蓝五处图标一致、自选/取消/刷新、旧填充字段不渲染、分组及真实 JSON 往返，无页面异常。截图和真实文件证据位于 `/tmp/virtual-net-ux-qa`。

2026-09-08 上一版自动检查（强调色收窄前的历史记录）：`pnpm -r exec vitest run --maxWorkers=2`，引擎 27 文件 / 223 项、网页 37 文件 / 188 项，合计 **411 项全部通过**；`pnpm lint` 检查 269 文件通过，`pnpm build` 含类型检查通过。保留已有的单包超过 500 kB 提示。上一版主流程脚本回归通过（当时包含主题取消、设备/连线覆盖与恢复、手机选择建组），无页面错误。早期资源争抢导致过单项默认 5 秒超时，最终完整运行使用原默认超时通过，未跳过测试或放宽断言。上述为旧版历史数量，本轮最终结果以 415 项记录为准。

## CP5 第四批：UDP 回显服务

依据：[批次细化](checkpoints/CP5-udp-echo.md)。

| 编号 | 验证内容 | 状态 |
| --- | --- | --- |
| T-CP5E-001 | 服务类型、同协议端口冲突、DNS 端口保留、旧文件与保存往返 | 通过：udpEcho.test.ts；Chrome 实际导出文件后重新导入 |
| T-CP5E-002 | 直连和代理回显、TCP/UDP 隔离、未开放服务与非法输入 | 通过：引擎测试；Chrome 直连/代理回显与停服定位 |
| T-CP5E-003 | 代理认证与关联、入口/出口/回程拦截、NAT、DNS 回归 | 通过：udpEcho.test.ts、原有 socksDns.test.ts 全部回归 |
| T-CP5E-004 | 两个入口、独立输入、服务配置与撤销、结果、保存和窄屏 | 通过：4 项新增组件交互测试；Chrome 桌面、390 × 844、刷新与导入后重新验证 |

## CP5 第三批：设备级网站访问分流

依据：[批次细化](checkpoints/CP5-routing.md)。

| 编号 | 验证内容 | 状态 |
| --- | --- | --- |
| T-CP5R-001 | 旧文件兼容、规则保存恢复、非法配置拒绝 | 通过：trafficRouting.test.ts，实际页面刷新保留规则 |
| T-CP5R-002 | 首条匹配、停用、覆盖、域名/IP/端口、失败不回退、其他验证不受影响 | 通过：15 项引擎测试 |
| T-CP5R-003 | 规则编辑排序、两个入口、结果、保存恢复、窄屏 | 通过：3 项新增组件交互测试、Chrome 桌面及 390 × 844 验证 |

## CP5 第二批：SOCKS5 UDP 与 DNS

依据：[批次细化](checkpoints/CP5-udp-dns.md)。状态：本批验证通过，不修改首批历史验收。

| 编号 | 检查 | 预期 | 结果 |
| --- | --- | --- | --- |
| T-CP5U-001 | 经 SOCKS5 查询外部 DNS | 控制、认证、关联、UDP 入口、DNS 出口和 UDP 返回逐段成功 | 通过：socksDns.test.ts；实际页面六段查询及路径动画 |
| T-CP5U-002 | 关闭代理、错误协议或认证 | 对应阶段失败，无出口查询与直连回退 | 通过：socksDns.test.ts；网页所选代理协议变更回归 |
| T-CP5U-003 | UDP 关闭、缺配置或非法端口 | 关联失败或导入/表单拒绝，无虚假入口 | 通过：引擎与组件测试；页面关闭、定位并重新开启 |
| T-CP5U-004 | 分别阻断 UDP 入口、DNS 出口 | 定位对应设备规则，不混用内外层地址与端口 | 通过：socksDns.test.ts，精确规则定位与内外层独立匹配 |
| T-CP5U-005 | 非状态规则阻断 UDP 返回 | 出口已获得答案，客户端仍查询失败 | 通过：socksDns.test.ts，客户端 dns 为空，返回段失败 |
| T-CP5U-006 | 内部 DNS、应答改写、不存在域名、无 DNS 地址 | 沿指定查询路径执行，分别返回有效答案或明确失败 | 通过：socksDns.test.ts；手机宽度内部 DNS 查询 |
| T-CP5U-007 | 直连 DNS、ping、三种网站代理回归 | 保持既有行为，不因 UDP 配置自动改道 | 通过：全量原有测试与新直接查询测试 |
| T-CP5U-008 | 保存、刷新、JSON 往返和旧文件 | 新配置保留，旧文件缺配置默认关闭 | 通过：旧文件自动测试；实际冷刷新、导出与选择文件导入后端口 2053 保留且查询成功 |
| T-CP5U-009 | 单节点及总验证窗口，桌面/窄屏 | 能选择 DNS 服务器与 SOCKS5、运行、查看各段及失败定位 | 通过：DnsOptions.interaction.test.ts 4 项；桌面与 390×844 页面查询、失败定位和截图检查 |
| T-CP5U-010 | 全量测试、lint、类型检查及构建 | 全部通过 | 通过：165 项引擎 + 117 项网页，共 282 项；lint、类型与构建通过，保留既有包体积提示 |
...
- 节点库分组调整：三类数量为 6/2/1，共 7 个职责小组、9 种原有实体；新增目录和组件测试覆盖唯一归属、别名、空结果、折叠与搜索展开、添加和单步撤销。实际浏览器在 1440×900 与 390×844 下验证折叠、键盘操作、SOCKS/DNS 搜索、点击与拖放添加和撤销，无横向溢出，测试后恢复原 8 节点。全量测试通过（引擎 145 项、网页 113 项，共 258 项），lint、类型检查和构建通过；仍有既有的 500 kB 包体积提示。
- 后续试用微调：步骤标记普通间距由 36 缩为 18，仅顶部有 VLAN 标签时保留 32；按钮由 22px 缩为 16px，内边距改为 1px。实际浏览器截图和序号详情点击通过；新增顶部 VLAN 标签与换侧间距测试。下方 248 项记录为此次微调前的整体验收证据。
...
- 冷刷新验收须先等待“已保存”，不能在 500ms 自动保存等待期内由自动化强制刷新。最终重跑：确认已保存后刷新，再实际导出，结构化比对原文件，全部节点、连线、曲线、端口侧、策略及模拟凭据一致（仅视角可不同）。
