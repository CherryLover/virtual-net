...
> 2026-09-07：CP0–CP3 保留既有验收记录；原 CP4 53 条用例为历史设计，待按通用网络方向修订，不能据此验收新 CP4。新增方向与 UX 验收候选见下节，全部未执行。

## 方向调整与页面优化验收候选

来源：[方向与页面优化](design/network-model-and-ui.md)。以下为验收候选，不是完成记录。`R-20260907-*` 是稳定候选编号；详细方案确认后分配正式 CP / UX 编号并保留映射，不覆盖原有编号。涉及待定方案的候选应先定稿预期，再执行。

| 候选编号 | 方式 | 操作与预期 | 来源 | 状态 |
| --- | --- | --- | --- | --- |
| R-20260907-01 | 页面操作 | 添加网络与访问控制设备，默认命名中性；改变节点位置不改变访问结果 | 第 2–3 节 | 未执行 |
| R-20260907-02 | 引擎测试 | 串联两台访问控制设备，分别拒绝同一请求时均定位到对应设备；另放不在路径上的设备不影响结果 | 第 3 节 | 未执行 |
| R-20260907-03 | 引擎测试 | 允许、拒绝规则冲突以及无命中时，按已定稿的规则顺序、默认动作和返回连接策略得出结果 | 第 3 节 | 待规则定稿 |
| R-20260907-04 | 引擎测试 | 分别由 DNS 服务改写、查询路径设备改写，原因指向实际处理者；改写后连另一有效服务不被强判连接失败 | 第 3 节 | 未执行 |
| R-20260907-05 | 页面操作 | 从左侧拖入“代理”，可配置入口、连线、选择使用、保存和重新打开；不是无功能的占位节点 | 第 4 节 | 未执行 |
| R-20260907-06 | 引擎测试 / 页面操作 | 通过首批支持的代理访问目标，展示客户端到代理和代理到目标的路径与返回结果 | 第 4 节 | 待协议范围定稿 |
| R-20260907-07 | 引擎测试 | 入口关闭、认证错误、代理解析失败、出口受限，分别产生正确原因与定位 | 第 4 节 | 未执行 |
| R-20260907-08 | 引擎测试 | 本地解析与代理解析显示各自路径；应用代理不改变普通 ping；未使用的代理不影响直连或取消告警 | 第 4 节 | 未执行 |
| R-20260907-09 | 页面操作 | 左栏搜索、分组、收起与添加流程可用；长名称和窄窗口不挤压工具栏或遮挡画布 | 第 5 节 | 未执行 |
| R-20260907-10 | 页面操作 | 切换节点、分组、验证结果后内容正确；诊断定位自动展开相应配置并定位字段 | 第 5 节 | 未执行 |
| R-20260907-11 | 页面操作 | 输入半个地址或错误配置时按统一提交规则处理；切换节点不串值，已保存状态准确 | 第 5 节 | 待提交方式定稿 |
| R-20260907-12 | 页面操作 | 选中连线后拖动曲线控制柄绕开节点，标签与高亮正常；恢复自动线形可用，拖柄不误移动节点或画布 | 第 6 节 | 未执行 |
| R-20260907-13 | 网页测试 / 页面操作 | 调整线形、缩放平移、移动单端与双端节点，曲线按定稿规则跟随，往返动画始终沿实际连线 | 第 6 节 | 待控制点规则定稿 |
| R-20260907-14 | 网页测试 / 页面操作 | 一次曲线拖动一次撤销；重做、刷新、导出再导入保留线形；旧文件恢复默认线形 | 第 6–7 节 | 未执行 |
| R-20260907-15 | 网页测试 / 页面操作 | 修改线形或端口显示侧不改变模拟结果，不让既有验证过期；真正换接端口仍使旧结果过期 | 第 6–7 节 | 未执行 |
| R-20260907-16 | 页面操作 | 普通 8 口交换机上、下均可方便接线，同级连接无强制的大幅底部绕行；已接端口不能重复占用 | 第 6 节 | 待端口布局定稿 |
| R-20260907-17 | 网页测试 / 页面操作 | 旧网络图端口身份、VLAN 与连线完整保留；高端口数节点和标签无重叠或无限扩宽 | 第 6–7 节 | 未执行 |
| R-20260907-18 | 页面操作 | 在 1440×900、1024×768 与 390×844 检查布局；桌面完整流程可用，窄屏符合定稿支持范围，无文字或控件溢出 | 第 5、8 节 | 待窄屏范围定稿 |
| R-20260907-19 | 命令 / 页面操作 | 实施后运行 lint、typecheck、test、build；回归 CP1 连通与失败、CP2 VLAN、CP3 路径播放和旧图导入 | 第 7–8 节 | 未执行 |
