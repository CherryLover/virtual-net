...
### 6.1 用户试用后的调整（2026-09-04）

**从设备直接发起验证**。选中电脑或路由器时，配置表单最上方增加「验证」块：
- 三个一键按钮：`ping 网关`（目标 = 该设备的网关；路由器取 WAN 网关）、`ping 外网`（目标 `8.8.8.8`）、`打开网站`（下拉选目标库域名，默认 `www.google.com`；路由器无此按钮）
- 一个目标输入框 + `ping` 按钮，填任意 IP
- 点击即运行，起点固定为当前设备。结果直接显示在这块下面：结论、原因、路径、逐跳列表、「定位」，与结果面板的「最近验证」是同一个组件
- 顶部工具栏的「验证」弹窗保留，用于任意起点

**设备图标**。每种设备节点名称左侧放一个线条风格 SVG 图标（内联，不引图标库）：电脑显示器、路由器带天线的盒子、互联网云。设备栏卡片同样带图标。CP2 的交换机、AP、光猫各补一个。
