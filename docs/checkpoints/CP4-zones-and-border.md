...
> **2026-09-07：本方案已暂停作为实施依据，需按新方向重写。** 当前范围以 [ROADMAP](../../ROADMAP.md) 和 [方向与页面优化](../design/network-model-and-ui.md) 为准。下文保留历史设计，不代表当前已确认方案。
> 已替代的内容包括特定地域与设备命名、全局唯一边界、地域直接放行、DNS 读取全局边界配置、固定协议放行结论以及“存在代理就取消告警”。原验收编号保留但待修订，不可照旧验收后宣告新 CP4 完成。
