...
66 条用例全部通过。独立复验：`pnpm lint / typecheck / test / build` 全绿（19 个测试文件 98 条，CP1 的 51 条一行未改）；用脚本驱动浏览器导入 `home-office.json`，从无线电脑3 一键 ping 外网，路径 电脑3 → AP1 → 交换机1 → 路由器1 → 光猫1 → 互联网 → 原路返回，逐跳含二层转发与 NAT，控制台无报错。与文档的偏差：

引擎
- WAN 与 LAN 撞段（`WAN_LAN_OVERLAP` / L022）只在上游是本地三层设备（路由模式光猫或另一台路由器）时判定，上游直接是互联网时保持 CP1 行为，否则 CP1 的 T-CP1-022 会被改判
- `ignoreVlan` 走图实现为「照样按 VLAN 规则判断并记丢弃点，但不真的丢弃」；丢弃点取通往目标那条路径上的第一处
- VLAN 断开导致目标连地址都拿不到时，先报 VLAN 原因而不是 `ARP_MISS`（场景 3 第 11 步需要）
- `static` WAN 也写一条租约（`via: 'static'`），否则路由表拿不到网关
- L017 的 `targets` 是两根线的四个端点，`linkId` 放第一根
- 路由器网桥与光猫的透明转发 note 带「二层转发，未路由」前缀，交换机与 AP 用「学习 … 泛洪 …」格式
- `br-lan` 接口的 `vlan` 取 1；AP 与桥接光猫内部按线上标签归 VLAN
- 新建路由器写 `vlans: []`、新建互联网写 `access.mode: 'dhcp'`，解析旧文件不补

网页
- 路由器 VLAN 行补了「VLAN 号」输入框（文档表格有此列但原实现漏了，场景 3 的 VLAN 10/20 靠它）；改号时若网段仍是默认值则跟着变
- 撤销快捷键只在真正可打字的控件上让开，复选框与下拉框上按 Ctrl/Cmd+Z 仍可撤销
- 端口表列宽重排，对端列长名字省略号加悬停

遗留
- 「纵向等距」在节点已重叠时会叠在一起（间距为负），边界情况未处理
- 引擎逐跳文案里从无 VLAN 口发出时会出现「VLAN 1 → VLAN -」，待 CP3 做解释文案时统一
- 页面无 favicon，控制台一条 404（CP1 起就有）
- L015 的成员判断是简化遍历，多层嵌套拓扑可能漏报
